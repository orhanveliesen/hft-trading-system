...
public:
    SMACrossover(int fast_period = 10, int slow_period = 30)
        : fast_period_(fast_period), slow_period_(slow_period), prev_fast_(0), prev_slow_(0) {}
...
private:
    ...
    double calculate_sma(int period) const {
        if (closes_.size() < static_cast<size_t>(period))
            return 0;
...
public:
    RSIStrategy(int period = 14, double oversold = 30, double overbought = 70)
        : period_(period), oversold_(oversold), overbought_(overbought), prev_close_(0) {}
...
public:
    MeanReversion(int lookback = 20, double std_multiplier = 2.0)
        : lookback_(lookback), std_multiplier_(std_multiplier) {}
...
public:
    explicit BreakoutStrategy(int lookback = 20) : lookback_(lookback) {}
...
public:
    MACDStrategy(int fast = 12, int slow = 26, int signal = 9)
        : fast_period_(fast), slow_period_(slow), signal_period_(signal), fast_ema_(0), slow_ema_(0), signal_ema_(0),
          prev_macd_(0), prev_signal_(0), initialized_(false) {}
