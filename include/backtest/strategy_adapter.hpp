...
public:
    explicit SimpleMRAdapter(const strategy::SimpleMRConfig& config = {})
        : strategy_(config)
        , position_(0)
    {}
    ...
    Signal on_kline(const exchange::Kline& kline, const BacktestPosition& pos) override {
        ...
        if (pos.is_short()) position_ = -position_;
        ...
        // Dönüştür
        switch (sig) {
            case strategy::Signal::Buy:  return Signal::Buy;
            case strategy::Signal::Sell: return Signal::Sell;
...
public:
    explicit MomentumAdapter(const strategy::MomentumConfig& config = {})
        : strategy_(config)
        , position_(0)
    {}
    ...
    Signal on_kline(const exchange::Kline& kline, const BacktestPosition& pos) override {
        ...
        switch (sig) {
            case strategy::MomentumSignal::Buy:  return Signal::Buy;
            case strategy::MomentumSignal::Sell: return Signal::Sell;
            default: return Signal::None;
        }
    }

    int64_t current_momentum() const {
        return strategy_.current_momentum_bps();
    }
...
public:
    ...
    explicit GenericStrategyAdapter(Args&&... args)
    ...
    Signal on_kline(const exchange::Kline& kline, const BacktestPosition& pos) override {
        ...
        if (static_cast<int>(sig) == 1) return Signal::Buy;   // Buy
        if (static_cast<int>(sig) == 2) return Signal::Sell;  // Sell
