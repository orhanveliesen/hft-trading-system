...
public:
    ...
    Quote generate_quotes(Price mid_price, int64_t position) const {
        ...
        if (half_spread == 0) half_spread = 1;  // Minimum 1 tick
        ...
        Price skew = static_cast<Price>(
            half_spread * position_ratio * config_.skew_factor
        );
        ...
        quote.bid_size = static_cast<Quantity>(
            std::min(static_cast<int64_t>(config_.quote_size), std::max(0L, room_to_buy))
        );
        quote.ask_size = static_cast<Quantity>(
            std::min(static_cast<int64_t>(config_.quote_size), std::max(0L, room_to_sell))
        );
