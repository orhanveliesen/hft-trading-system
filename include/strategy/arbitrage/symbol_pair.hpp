...
struct SymbolPair {
    ...
    static std::optional<SymbolPair> parse(const std::string& symbol) {
        ...
        static const std::vector<std::string> known_quotes = {
        ...
        for (const auto& quote : known_quotes) {
            if (upper.size() > quote.size() &&
                upper.substr(upper.size() - quote.size()) == quote) {
    ...
    static bool shares_currency(const SymbolPair& a, const SymbolPair& b) {
        return a.base == b.base || a.base == b.quote ||
               a.quote == b.base || a.quote == b.quote;
    }
    ...
    static std::optional<std::string> common_currency(const SymbolPair& a, const SymbolPair& b) {
        ...
        if (a.base == b.quote) return a.base;
        if (a.quote == b.base) return a.quote;
        if (a.quote == b.quote) return a.quote;
        return std::nullopt;
    }
};
