...
public:
    explicit EventPublisher(bool enabled = true) : enabled_(enabled) {
        if (enabled_) {
            try {
                ...
                std::cout << "[IPC] Event publisher initialized (buffer: " << buffer_->capacity() << " events)\n";
    ...
    void status(uint32_t sym, const char* ticker, StatusCode code, double price = 0, uint8_t sig_strength = 0,
                uint8_t regime = 0) {
        if (!enabled_)
            return;
        auto ts = now_ns();
...
private:
    ...
    static uint64_t now_ns() { return std::chrono::steady_clock::now().time_since_epoch().count(); }
};
