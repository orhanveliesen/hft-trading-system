...
struct SymbolEventStats {
    ...
    bool matches(const char* sym) const {
        return std::strcmp(symbol, sym) == 0;
    }
    ...
    double total_pnl() const {
        return total_pnl_x100.load() / 100.0;
    }

    double session_pnl() const {
        return session_pnl_x100.load() / 100.0;
    }
...
struct TunerStats {
    ...
    double total_cost() const {
        return total_cost_x100.load() / 100.0;
    }
};
...
struct SharedEventLog {
    ...
    static SharedEventLog* create(const char* name = EVENT_LOG_SHM_NAME) {
        ...
        void* ptr = mmap(nullptr, sizeof(SharedEventLog),
    ...
    static void destroy(const char* name = EVENT_LOG_SHM_NAME) {
        shm_unlink(name);
    }
