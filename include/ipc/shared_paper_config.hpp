...
struct SharedPaperConfig {
    ...
    // === Accessors ===
    double slippage_bps() const { return slippage_bps_x100.load(std::memory_order_relaxed) / 100.0; }

    int64_t get_simulated_latency_ns() const { return simulated_latency_ns.load(std::memory_order_relaxed); }

    double fill_probability() const { return fill_probability_x10000.load(std::memory_order_relaxed) / 10000.0; }
    ...
    static SharedPaperConfig* create(const char* name) {
        ...
        void* ptr = mmap(nullptr, sizeof(SharedPaperConfig), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
        close(fd);
    ...
    static SharedPaperConfig* open(const char* name) {
        ...
        void* ptr = mmap(nullptr, sizeof(SharedPaperConfig), PROT_READ, MAP_SHARED, fd, 0);
        close(fd);
