...
struct TunerDecision {
    ...
    bool add_change(TunerParam p, float old_v, float new_v) {
        if (num_changes >= MAX_PARAM_CHANGES) return false;
    ...
    // Accessors
    TunerConcern get_concern() const {
        return static_cast<TunerConcern>(concern);
    }

    const char* get_concern_name() const {
        return TradeEvent::concern_name(get_concern());
    }
...
struct SharedTunerState {
    ...
    const TunerDecision* get_latest() const {
        ...
        if (d.sequence == 0) return nullptr;
        return &d;
    }
    ...
    const TunerDecision* get_by_offset(size_t offset) const {
        if (offset >= MAX_TUNER_HISTORY) return nullptr;
    ...
    static SharedTunerState* create(const char* name = SHM_NAME) {
        ...
        void* ptr = mmap(nullptr, sizeof(SharedTunerState),
     ...
     */
    static void destroy(const char* name = SHM_NAME) {
        shm_unlink(name);
    }
};
