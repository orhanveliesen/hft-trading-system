...
public:
    ...
    virtual uint64_t send_market_order(Symbol symbol, Side side, double qty, Price expected_price) = 0;
...
public:
    ...
    using FillCallback =
        std::function<void(uint64_t order_id, Symbol symbol, Side side, double qty, Price price, double slippage)>;
    ...
    explicit ExecutionEngine(const ExecutionConfig& config = {}) : config_(config) {
     ...
     */
    uint64_t execute(Symbol symbol, const Signal& signal, const MarketSnapshot& market, MarketRegime regime) {
...
private:
     ...
     */
    OrderType decide_order_type(const Signal& signal, const MarketSnapshot& market, MarketRegime regime) {
     ...
     */
    Price calculate_limit_price(const Signal& signal, const MarketSnapshot& market, Side side) {
