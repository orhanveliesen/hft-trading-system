...
struct EnterOrder {
    ...
    void set_quantity(uint32_t qty) { write_be32(reinterpret_cast<uint8_t*>(&quantity), qty); }

    void set_price(uint32_t p) { write_be32(reinterpret_cast<uint8_t*>(&price), p); }

    void set_time_in_force(uint32_t tif) { write_be32(reinterpret_cast<uint8_t*>(&time_in_force), tif); }

    void set_min_quantity(uint32_t min_qty) { write_be32(reinterpret_cast<uint8_t*>(&min_quantity), min_qty); }
};
