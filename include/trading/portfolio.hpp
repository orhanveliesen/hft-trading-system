...
struct Portfolio {
    ...
    // === Config Accessors ===
    // Read from SharedConfig or use centralized defaults
    ...
    // Symbol-aware position sizing: checks symbol-specific config first
    ...
    void set_symbol_configs(const ipc::SharedSymbolConfigs* cfgs) { symbol_configs_ = cfgs; }
    ...
    void reserve_cash(double amount) { pending_cash += amount; }
