...
TEST(fractional_btc_quantity_through_callback) {
    ...
    adapter.set_fill_callback(
        [](uint64_t order_id, const char* symbol_name, Side side, double qty, Price fill_price, double commission) {
    ...
    uint64_t order_id = adapter.send_limit_order(sym_id, Side::Buy, fractional_qty, limit_price_scaled);
...
TEST(very_small_eth_quantity) {
    ...
    adapter.set_fill_callback([](uint64_t, const char* symbol, Side side, double qty, Price, double) {
        received_qty = qty;
    ...
    adapter.send_limit_order(sym_id, Side::Buy, tiny_qty, adapter.double_to_price(limit_price));

    // Trigger fill
    adapter.on_price_update(sym_id, adapter.double_to_price(2895.0), adapter.double_to_price(2898.0), 1000000);
...
TEST(typical_thousand_dollar_btc_trade) {
    ...
    adapter.set_fill_callback([](uint64_t, const char*, Side, double qty, Price, double) {
        received_qty = qty;
        callback_count++;
    });
    ...
    adapter.send_limit_order(sym_id, Side::Buy, btc_qty, adapter.double_to_price(btc_price));

    // Trigger fill
    adapter.on_price_update(sym_id, adapter.double_to_price(86900.0), adapter.double_to_price(86950.0), 1000000);
...
TEST(sell_order_fractional_quantity) {
    ...
    adapter.set_fill_callback([](uint64_t, const char*, Side side, double qty, Price, double) {
        received_qty = qty;
        received_side = side;
        callback_count++;
    });
    ...
    adapter.send_limit_order(sym_id, Side::Sell, fractional_qty,
                             adapter.double_to_price(88000.0) // Sell at 88000
    );

    // Bid rises above limit - fill
    adapter.on_price_update(sym_id, adapter.double_to_price(88100.0), adapter.double_to_price(88150.0), 1000000);
