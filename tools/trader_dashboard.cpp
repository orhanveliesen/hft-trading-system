...
inline ImVec4 regime_color(MarketRegime regime) {
    ...
    case MarketRegime::Ranging:
        return ImVec4(0.9f, 0.9f, 0.2f, 1.0f); // Yellow
    case MarketRegime::HighVolatility:
        return ImVec4(1.0f, 0.5f, 0.0f, 1.0f); // Orange
    case MarketRegime::LowVolatility:
        return ImVec4(0.5f, 0.5f, 0.8f, 1.0f); // Blue-gray
    default:
...
struct DashboardData {
    ...
    void check_connection_alerts(const SharedConfig* config) {
        ...
        if (last_trader_start_time_ns != 0 && start_time != 0 && start_time != last_trader_start_time_ns) {
            add_alert("Trader Engine Restarted - Session recovered", false,
                      ImVec4(0.4f, 0.8f, 1.0f, 1.0f)); // Blue/info
        }
        ...
        if (ws_market != last_ws_market_status) {
            if (ws_market == 0 && last_ws_market_status == 2) {
                ...
                add_alert("CONNECTION LOST - Reconnecting...", true, ImVec4(1.0f, 0.2f, 0.2f, 1.0f)); // Red
            } else if (ws_market == 1 && last_ws_market_status == 2) {
                // Healthy -> Degraded
                add_alert("Connection Degraded - No data received", true, ImVec4(1.0f, 0.6f, 0.0f, 1.0f)); // Orange
            } else if (ws_market == 2 && last_ws_market_status < 2) {
                // Reconnected
                add_alert("Connection Restored", false, ImVec4(0.2f, 1.0f, 0.2f, 1.0f)); // Green
            }
            last_ws_market_status = ws_market;
        }
    }

    DashboardData() : start_time(std::chrono::steady_clock::now()), last_pnl_sample(std::chrono::steady_clock::now()) {}
    ...
    void load_from_shared_state(const SharedPortfolioState* state) {
        ...
        for (size_t i = 0; i < MAX_PORTFOLIO_SYMBOLS; ++i) {
            ...
            std::cout << "[DEBUG] Slot " << i << ": " << sym << " qty=" << qty << " avg=$" << avg << " last=$" << last
                      << "\n";
        ...
        std::cout << "[IPC] Loaded: active_slots=" << active_slots << ", loaded_positions=" << loaded_positions
    ...
    double total_equity() const { return current_cash + total_market_value(); }
    ...
    void process_event(const TradeEvent& e) {
        ...
        case EventType::Fill: {
            ...
            snprintf(buf, sizeof(buf), "%6.1fs  %s  %s  %.0f @ $%.4f  [#%u]", rel_sec, e.side == 0 ? "BUY " : "SELL",
                     symbol.c_str(), e.quantity, e.price, e.order_id);
            // Green for BUY, Yellow for SELL
            events.push_front(
                {buf, e.side == 0 ? ImVec4(0.2f, 0.9f, 0.2f, 1.0f) : ImVec4(0.9f, 0.9f, 0.2f, 1.0f), rel_sec});
            break;
        }
        case EventType::TargetHit: {
            ...
            snprintf(buf, sizeof(buf), "%6.1fs  TARGET %s  %.0f @ $%.4f  +$%.2f", rel_sec, symbol.c_str(), e.quantity,
                     e.price, pnl);
        ...
        case EventType::StopLoss: {
            ...
            snprintf(buf, sizeof(buf), "%6.1fs  STOP   %s  %.0f @ $%.4f  $%.2f", rel_sec, symbol.c_str(), e.quantity,
                     e.price, pnl);
            // Red for loss
        ...
        case EventType::Signal: {
            snprintf(buf, sizeof(buf), "%6.1fs  SIGNAL %s  %s", rel_sec, symbol.c_str(), e.side == 0 ? "BUY" : "SELL");
        ...
        case EventType::RegimeChange: {
            ...
            snprintf(buf, sizeof(buf), "%6.1fs  REGIME %s  -> %s", rel_sec, symbol.c_str(),
        ...
        case EventType::Status: {
            ...
            if (e.price > 0) {
                snprintf(buf, sizeof(buf), "%6.1fs  %s  %s  $%.2f", rel_sec, code_name, symbol.c_str(), e.price);
            } else {
                snprintf(buf, sizeof(buf), "%6.1fs  %s  %s", rel_sec, code_name, symbol.c_str());
            }
            ...
            } else if (code == StatusCode::AutoTuneCooldown || code == StatusCode::AutoTuneSignal ||
                       code == StatusCode::AutoTuneMinTrade) {
                color = ImVec4(1.0f, 0.8f, 0.2f, 1.0f); // Orange - warning
            } else if (code == StatusCode::AutoTunePaused || code == StatusCode::DrawdownAlert ||
                       code == StatusCode::VolatilitySpike || code == StatusCode::CashLow) {
        ...
        case EventType::TunerConfig: {
            ...
            snprintf(buf, sizeof(buf), "%6.1fs  %s  %s  [%d%% conf]", rel_sec, code_name, symbol.c_str(), confidence);
...
void render_dashboard(DashboardData& data, const SharedPortfolioState* portfolio_state, SharedConfig* config,
                      SharedPaperConfig* paper_config, SharedSymbolConfigs* symbol_configs) {
    auto now = std::chrono::steady_clock::now();
    ...
    if (!data.active_alerts.empty()) {
        ...
        if (!alert.acknowledged) {
            ...
            bool flash = alert.is_critical && (static_cast<int>(ImGui::GetTime() * 2.0f) % 2 == 0);

            ImVec4 bg_color = flash ? ImVec4(1.0f, 0.1f, 0.1f, 0.9f) : // Bright red flash
    ...
    if (portfolio_state) {
        ...
        double pnl_pct = portfolio_state->initial_cash() > 0
                             ...
                             : 0.0;
        ImVec4 equity_color = total_equity >= portfolio_state->initial_cash() ? ImVec4(0.2f, 1.0f, 0.2f, 1.0f)
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, equity_color);
        ImGui::Text("%s$%.2f (%s%.2f%%)", pnl_dollars >= 0 ? "+" : "", pnl_dollars, pnl_pct >= 0 ? "+" : "", pnl_pct);
        ImGui::PopStyleColor();
    ...
    if (!data.pnl_history.empty()) {
        ...
        ImGui::PlotLines("##equity", history.data(), history.size(), 0, nullptr, -range, range, ImVec2(-1, 90));
    ...
    if (ImGui::BeginTable("positions_table", 9,
        ...
        for (const auto& [sym, pos] : data.positions) {
            ...
            if (it != data.price_history.end() && !it->second.empty()) {
                ...
                ImGui::PlotLines("##spark", prices.data(), prices.size(), 0, nullptr, min_p, max_p, ImVec2(100, 20));
            }
    ...
    if (config && ImGui::CollapsingHeader("STRATEGY CONFIG", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            if (trader_status == 2 && is_alive) {
                ...
                if (ImGui::IsItemHovered()) {
                    ImGui::SetTooltip("Git commit: %.8s\nPID: %d\nMode: %s\nTuner: %s\nManual: %s",
                                      config->get_build_hash(), config->get_trader_pid(),
                                      is_paper ? "Paper Trading" : "Live Trading", is_tuner_mode ? "ON" : "OFF",
                                      is_manual ? "ON" : "OFF");
                }
                ...
                if (ImGui::IsItemHovered()) {
                    ImGui::SetTooltip(
                        "Trader engine is not responding.\nLast known PID: %d\nPossible crash or kill -9.",
        ...
        // ===== LIVE STATUS (from Trader) =====
        {
            ...
            if (active > 5)
                active = 2; // Default to NORMAL if invalid
        ...
        if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip("STRATEGY MODE\n\n"
        ...
        if (ImGui::TreeNode("Regime Strategy Mapping")) {
            ...
            const char* strategy_names[] = {"NONE",      "MOMENTUM", "MEAN_REV", "MKT_MAKER",
                                            "DEFENSIVE", "CAUTIOUS", "SMART"};
            const char* regime_names[] = {"Unknown", "TrendingUp", "TrendingDown", "Ranging",
                                          "HighVol", "LowVol",     "Spike"};
            ...
            if (ImGui::BeginTable("RegimeStrategyTable", 2, ImGuiTableFlags_BordersInner | ImGuiTableFlags_RowBg)) {
                ...
                for (int i = 0; i <= 6; ++i) {
                    ...
                    switch (i) {
                    case 1:
                        regime_color = ImVec4(0.2f, 1.0f, 0.2f, 1.0f);
                        break; // TrendingUp - green
                    case 2:
                        regime_color = ImVec4(1.0f, 0.3f, 0.3f, 1.0f);
                        break; // TrendingDown - red
                    case 3:
                        regime_color = ImVec4(0.6f, 0.8f, 1.0f, 1.0f);
                        break; // Ranging - light blue
                    case 4:
                        regime_color = ImVec4(1.0f, 0.5f, 0.0f, 1.0f);
                        break; // HighVol - orange
                    case 5:
                        regime_color = ImVec4(0.5f, 0.5f, 0.8f, 1.0f);
                        break; // LowVol - purple
                    case 6:
                        regime_color = ImVec4(1.0f, 0.0f, 0.5f, 1.0f);
                        break; // Spike - pink
                    default:
                        regime_color = ImVec4(0.5f, 0.5f, 0.5f, 1.0f); // Unknown - gray
                    }
        ...
        if (ImGui::TreeNode("Risk Management")) {
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("REGIME CHECK THRESHOLD\n\n"
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("MAX DRAWDOWN LIMIT\n\n"
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("CONSECUTIVE LOSS LIMIT\n\n"
        ...
        if (ImGui::TreeNode("Default Parameters")) {
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("MINIMUM POSITION\n\n"
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("MAXIMUM POSITION\n\n"
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("TREND EXIT (Pullback from Peak)\n\n"
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("RISK:REWARD RATIO\n\n"
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("TRADE COOLDOWN\n\n"
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("MINIMUM TRADE SIZE\n\n"
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("SIGNAL STRENGTH FILTER\n\n"
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("EMA DEVIATION - TRENDING REGIME\n\n"
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("EMA DEVIATION - RANGING REGIME\n\n"
                                  "Max %% price can be above EMA\n"
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("EMA DEVIATION - HIGH VOLATILITY\n\n"
                                  "Max %% price can be above EMA\n"
        ...
        if (config->is_paper_trading() && ImGui::TreeNodeEx("Paper Trading Costs", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("COMMISSION RATE (per trade)\n\n"
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("SLIPPAGE SIMULATION (basis points)\n\n"
            ...
            ImGui::Separator();
            ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.2f, 1.0f), "Round-trip: %.3f%%", rt_total);
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("TOTAL ROUND-TRIP COST\n\n"
                                  ...
                                  "to be profitable!",
                                  commission, rt_commission, slippage, rt_slippage, rt_total);
            }
    ...
    if (config && symbol_configs && ImGui::CollapsingHeader("TUNER CONTROL", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip("AI TUNER MODE\n\n"
        ...
        if (tuner_on) {
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("PAUSE/RESUME TUNER\n\n"
                                  "When paused:\n"
        ...
        if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip("FORCE IMMEDIATE TUNING\n\n"
        ...
        if (ImGui::Begin(window_title, &data.show_symbol_config, ImGuiWindowFlags_NoCollapse)) {
            ...
            if (cfg) {
                ...
                if (ImGui::TreeNodeEx("EMA Deviation Thresholds", ImGuiTreeNodeFlags_DefaultOpen)) {
                    ...
                    float ema_trend = use_global_ema ? static_cast<float>(config->ema_dev_trending())
                                                     : cfg->ema_dev_trending_x100 / 100.0f;
                    float ema_range = use_global_ema ? static_cast<float>(config->ema_dev_ranging())
                                                     : cfg->ema_dev_ranging_x100 / 100.0f;
                    float ema_hvol = use_global_ema ? static_cast<float>(config->ema_dev_highvol())
                ...
                if (ImGui::TreeNodeEx("Position Sizing", ImGuiTreeNodeFlags_DefaultOpen)) {
                    ...
                    // Get values (global or symbol-specific)
                    float base_pos = use_global_pos ? static_cast<float>(config->base_position_pct())
                                                    : cfg->base_position_x100 / 100.0f;
                    float max_pos = use_global_pos ? static_cast<float>(config->max_position_pct())
                ...
                if (ImGui::TreeNodeEx("Target/Stop", ImGuiTreeNodeFlags_DefaultOpen)) {
                    ...
                    // Get values (global or symbol-specific)
                    float target =
                        use_global_target ? static_cast<float>(config->target_pct()) : cfg->target_pct_x100 / 100.0f;
                    float stop =
                        use_global_target ? static_cast<float>(config->stop_pct()) : cfg->stop_pct_x100 / 100.0f;
                    float pullback = use_global_target ? static_cast<float>(config->pullback_pct())
                ...
                if (ImGui::TreeNodeEx("Trade Filtering", ImGuiTreeNodeFlags_DefaultOpen)) {
                    ...
                    // Get values (global or symbol-specific)
                    int cooldown = use_global_filter ? config->get_cooldown_ms() : static_cast<int>(cfg->cooldown_ms);
                    int sig_str =
                        use_global_filter ? config->get_signal_strength() : static_cast<int>(cfg->signal_strength);
