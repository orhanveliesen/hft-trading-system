...
MatchingEngine::MatchingEngine(Price base_price, size_t price_range)
...
PriceLevel* MatchingEngine::allocate_level() {
...
void MatchingEngine::deallocate_level(PriceLevel* level) {
    level->next = free_levels_;
    free_levels_ = level;
}

OrderResult MatchingEngine::add_order(OrderId id, Side side, Price price, Quantity quantity,
                                       TraderId trader_id) {
    // Validate inputs
...
Quantity MatchingEngine::try_match_buy(Order* order) {
...
Quantity MatchingEngine::try_match_sell(Order* order) {
    Quantity remaining = order->quantity;
    Price limit_price = order->price;
...
void MatchingEngine::execute_trade(Order* aggressive, Order* passive, Quantity qty) {
...
bool MatchingEngine::would_self_trade(const Order* aggressive, const Order* passive) const {
...
void MatchingEngine::add_order_to_level(Order* order, PriceLevel* level) {
...
void MatchingEngine::remove_order_from_level(Order* order, PriceLevel* level) {
    if (order->prev) {
...
Quantity MatchingEngine::bid_quantity_at(Price price) const {
    return bids_.quantity_at(price);
}

Quantity MatchingEngine::ask_quantity_at(Price price) const {
